...
AICls <- function(n,np,SSres){
  #Error checking
  if(n<=0) {
    warning("n must be strictly positive")
    return(NA)
  }
  if(np< 0 || np > n) {
    warning("np must be positive and lower than n")
    return(NA)
  }
  if(SSres<0) {
    warning("SSres must be positive")
    return(NA)
  }
  #Main
...
#' @param Mmin minimum for the range of M, default value = MMIN
#' @param Mmax maximum or the range of M, default value = MMAX
...
least_square_M <- function(WS,SW,GC,cor=COR,snpthresh=SNPTHRESH,mincat=MINCAT,
                           Mmin=MMIN,Mmax=MMAX,
...
least_square_B <- function(WS,SW,GC,cor=COR,snpthresh=SNPTHRESH,mincat=MINCAT,
                           Bmin=BMIN,Bmax=BMAX,
                           Maxit=MAXIT,Factr=FACTR,Lmm=LMM,Verbose=VERBOSE,Usegr=USEGR) {
  # Determination of initial values for optimization: use of the simple regression
  #Error checking
...
least_square_BM <- function(WS,SW,GC,cor=COR,snpthresh=SNPTHRESH,mincat=MINCAT,
...
least_square_hotspot1 <- function(WS,SW,GC,cor=COR,snpthresh=SNPTHRESH,mincat=MINCAT,
                                  Bmin=BMIN,Bmax=BMAX,Mmin=MMIN,Mmax=MMAX,
                                  Maxit=MAXIT,Factr=FACTR,Lmm=LMM,Verbose=VERBOSE,Usegr=USEGR) {
  # Determination of initial values for optimization: use of the simple regression
  #Error checking
...
#' LS$f # proportion of hotspots
least_square_hotspot2 <- function(WS,SW,GC,cor=COR,snpthresh=SNPTHRESH,mincat=MINCAT,
...
#' LS$B0 # population-scaled background gBGC
#' LS$B1 # population-scaled hotspot gBGC
least_square_hotspot2bis <- function(WS,SW,GC,f,cor=COR,snpthresh=SNPTHRESH,mincat=MINCAT,
                                     B0min=BMIN,B0max=BMAX,B1min=BMIN,B1max=BMAX,Mmin=MMIN,Mmax=MMAX,
                                     Maxit=MAXIT,Factr=FACTR,Lmm=LMM,Verbose=VERBOSE,Usegr=USEGR) {
  #Error checking
  if(!is.numeric(c(WS,SW)) || length(which(c(WS,SW)<0))>0 ) {
